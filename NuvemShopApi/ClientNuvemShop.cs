...
namespace NuvemShopApi
{
    public class ClientNuvemShop
    {
        ...
        public const string ResourceDomainAuthorize = "https://www.api.tiendanube.com";
        ...
        public ICredentialsNuvemShop Credentials { get; }
        ...
        {
            var client = new RestClient(ResourceDomainAuthorize);

            var request = new RestRequest(ResourceUrlAuthorize, Method.POST);
        ...
        public T GetData<T>(string resource, params Parameter[] anotherParameters)
            => RunRequest<T>(resource, Method.GET, anotherParameters);

        public T PutData<T>(string resource, params Parameter[] anotherParameters)
            => RunRequest<T>(resource, Method.PUT, anotherParameters);

        public T PostData<T>(string resource, params Parameter[] anotherParameters)
            => RunRequest<T>(resource, Method.POST, anotherParameters);

        public T HeadData<T>(string resource, params Parameter[] anotherParameters)
            => RunRequest<T>(resource, Method.HEAD, anotherParameters);

        public T DeleteData<T>(string resource, params Parameter[] anotherParameters)
            => RunRequest<T>(resource, Method.DELETE, anotherParameters);


        private void UpdateLimitRequests(IRestResponse response)
        {
            LimitRequests =
                int.Parse(response.Headers.FirstOrDefault(r => r.Name.Equals("X-Rate-Limit-Limit"))?.Value.ToString() ??
                          "-1");
            LimitRemaining =
                int.Parse(
                    response.Headers.FirstOrDefault(r => r.Name.Equals("X-Rate-Limit-Remaining"))?.Value.ToString() ??
                    "-1");
            LimitReset =
                int.Parse(response.Headers.FirstOrDefault(r => r.Name.Equals("X-Rate-Limit-Reset"))?.Value.ToString() ??
                          "-1");
        }
